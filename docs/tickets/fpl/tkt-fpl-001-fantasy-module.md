...
- [ ] Read `FPL_REFRESH_TOKEN` and `FPL_ENTRY_ID` in the module, and document both in [`.env.example`](../../../.env.example). Leave them out of the platform env schema. Token exchange, refresh-token rotation, and one retry on 401/403 follow [FPL authentication](../../../src/modules/fpl/README.md)
